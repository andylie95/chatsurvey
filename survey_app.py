...
class SurveyApp:
    ...
    def display_question(self):
        if st.session_state.current_question < len(self.questions):
            ...
            if q_type == 'alternative':
                ...
                if st.button("Submit", key=f"submit{st.session_state.current_question}") and answer:
                    self.submit_answer(prompt, answer)
            elif q_type == 'rating':
    ...
    def save_conversation(self):
        ...
        st.download_button("Download Responses as CSV", data=csv_file, file_name='survey_responses.csv', mime='text/csv', key="download_csv")
