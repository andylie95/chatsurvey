    ...
    def display_question(self):
        if st.session_state.current_question < len(self.questions):
            ...
            if q_type == 'alternative':
                ...
                options = [f"1 = {question_row['option1']} (strongly disagree)", 
                           f"2 = {question_row['option2']} (disagree)", 
                           f"3 = {question_row['option3']} (neutral)", 
                           f"4 = {question_row['option4']} (agree)", 
                           f"5 = {question_row['option5']} (strongly agree)"]
                answer = st.radio("Your rating:", options, key=f"q{st.session_state.current_question}")
                if st.button("Next", key=f"next{st.session_state.current_question}"):
                    self.submit_answer(prompt, answer.split('=')[0].strip())
        else:
